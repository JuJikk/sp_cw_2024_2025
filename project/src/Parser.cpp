...
namespace Parser {
    ...
    void ifStatement() {
        ...
        // Перевіряємо, чи після умови стоїть ";"
        ...
        // Перевіряємо, чи є else
        if (TokenTable[pos].type == Else) {
            ++pos;  // споживаємо 'else'

            // Аналогічно перевіряємо, чи else-гілка не порожня
            if (TokenTable[pos].type == Semicolon) {
    ...
    void whileStatement() {
        ...
        // парсимо тіло while до 'END'
        while (TokenTable[pos].type != EndBlock) {
            statement();
        }

        // коли зустріли 'END', треба з’їсти цей токен...
